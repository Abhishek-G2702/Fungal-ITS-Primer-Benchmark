...
# ====================== CONFIGURATION ======================
FASTA_FILE = "pathogenic_fungi_sequences.fasta"  # Your input file
OUTPUT_DIR = "code1__Primer_Results"       # Main output directory
    ...
    for i in range(len(sequence) - len_primer + 1):
        ...
        for j in range(len_primer):
            if primer[j] != sequence[i + j]:
                mismatches += 1
                details.append(f"{primer[j]}â†’{sequence[i+j]}@pos{j+1}")
...
def main():
    ...
    for primer_name, primers in primer_pairs.items():
        print(f"ðŸ” Processing {primer_name}...")
...
ðŸ“‚ Results organized in '{OUTPUT_DIR}/' with:
